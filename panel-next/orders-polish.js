...
function dayISO(offset=0){ const date=new Date(); date.setDate(date.getDate()+offset); return new Date(date.getTime()-date.getTimezoneOffset()*60000).toISOString().slice(0,10); }
...
function installStyles(){
  ...
  const style=document.createElement('style'); style.id='ordersFinalPolishStyles'; style.textContent=`
#ordersView .orders-extra-head>div{display:grid;gap:3px;min-width:0}#ordersView .orders-extra-head small{display:block!important;line-height:1.15}#ordersView .orders-extra-head>div>strong{display:block!important;line-height:1.08;margin-top:1px}
#orderModal #orderProductName{display:none!important}#orderModal .order-product-original{display:none!important}#orderModal .order-auto-field{display:none!important}
#orderModal .order-product-buttons-wrap{grid-column:1/-1;margin:0 0 14px}#orderModal .order-product-buttons-title{font-size:13px;font-weight:1000;color:#737f79;text-transform:uppercase;margin:0 0 8px}
#orderModal .order-product-buttons{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}#orderModal .order-product-choice{border:2px solid #d9e4de;background:#fff;color:#233129;border-radius:12px;min-height:58px;padding:10px 9px;font-size:15px;font-weight:1000;line-height:1.12;text-align:center}
#orderModal .order-product-choice:nth-child(5n+1){background:#e8f8ee;border-color:#bfe4cd;color:#08713a}#orderModal .order-product-choice:nth-child(5n+2){background:#e8f4ff;border-color:#c7dff3;color:#155c91}#orderModal .order-product-choice:nth-child(5n+3){background:#fff4c9;border-color:#f1d77b;color:#6d5100}#orderModal .order-product-choice:nth-child(5n+4){background:#ffe8ef;border-color:#f1c7d4;color:#8a3453}#orderModal .order-product-choice:nth-child(5n+5){background:#eee8ff;border-color:#d8cef4;color:#543a8d}#orderModal .order-product-choice.active{background:#078844!important;border-color:#078844!important;color:#fff!important;box-shadow:0 5px 13px rgba(7,136,68,.18)}
#orderModal .order-fixed-info{grid-column:1/-1;display:flex;gap:8px;flex-wrap:wrap;margin:-3px 0 12px}#orderModal .order-fixed-info span{background:#f1f6f3;border:1px solid #dde7e2;border-radius:999px;padding:7px 10px;font-size:13px;font-weight:900;color:#425149}@media(max-width:520px){#orderModal .order-product-choice{font-size:14px;min-height:56px;padding:8px}}`;
  document.head.appendChild(style);
}
function findDuplicateDayContainer(button){let node=button.parentElement;const view=$('#ordersView');for(let depth=0;node&&node!==view&&depth<4;depth+=1,node=node.parentElement){if(node.id==='ordersDayChoices')return null;const buttons=$$('button',node),text=normalize(node.textContent);if(buttons.length>=2&&buttons.length<=4&&text.includes('pedidos hoy')&&text.includes('pedidos manana'))return node;}return null;}
function hideDuplicateDayButtons(){const view=$('#ordersView');if(!view)return;$$('button',view).forEach(button=>{if(button.closest('#ordersDayChoices')||!normalize(button.textContent).includes('pedidos hoy'))return;const container=findDuplicateDayContainer(button);if(container)container.style.setProperty('display','none','important');});}
function configuredProduct(name){const key=normalize(name);return MenuStore.list().find(item=>normalize(item.name)===key)||null;}
function productChoices(){const menu=MenuStore.list().filter(item=>String(item.name||'').trim());return menu.length?menu:[{name:'Ceviche de camarón'},{name:'Ceviche de pescado'},{name:'Ceviche mixto'},{name:'Ceviche pulpo y camarón'},{name:'Ceviche pulpo y pescado'}];}
function updateFixedInfo(){const host=$('#orderFixedInfo'),unitField=$('#orderUnit'),priceField=$('#orderPrice');if(!host||!unitField||!priceField)return;const product=configuredProduct($('#orderProductName')?.value||'');const unitLabel=unitField.closest('label'),priceLabel=priceField.closest('label');const hasUnit=Boolean(product?.unit),hasPrice=product?.price!==null&&product?.price!==undefined&&product?.price!=='';unitLabel?.classList.toggle('order-auto-field',hasUnit);priceLabel?.classList.toggle('order-auto-field',hasPrice);const bits=[];if(hasUnit)bits.push(`<span>Unidad: <b>${product.unit}</b></span>`);if(hasPrice)bits.push(`<span>Precio: <b>$${Number(product.price).toFixed(2)}</b></span>`);host.innerHTML=bits.join('');host.hidden=!bits.length;}
...
function syncOrderBaseFilters(day){
  const view=$('#ordersView');
  if(view&&day)view.dataset.selectedDay=day;
  const dayFilter=$('#orderDayFilter');
  if(dayFilter){
    dayFilter.value='all';
    dayFilter.dispatchEvent(new Event('change',{bubbles:true}));
  }
  const all=$('#statusTabs [data-status="all"]');
  if(all&&!all.classList.contains('active')) all.click();
}
function prefillOrderDate(){
  const selected=$('#ordersView')?.dataset.selectedDay;
  const field=$('#orderDate');
  if(!field||!selected)return;
  field.value=selected==='tomorrow'?dayISO(1):dayISO(0);
...
document.addEventListener('click',event=>{
  ...
  if(dayButton){
    const day=dayButton.dataset.localOrderDay;
    syncOrderBaseFilters(day);
    setTimeout(()=>syncOrderBaseFilters(day),0);
  }
  const productButton=event.target.closest('[data-order-product-choice]');
  if(productButton){event.preventDefault();event.stopPropagation();selectProduct(decodeURIComponent(productButton.dataset.orderProductChoice||''));return;}
  if(event.target.closest('#newOrderButton')){forceModalButtons();setTimeout(prefillOrderDate,0);return;}
  if(event.target.closest('[data-order-action="edit"]'))forceModalButtons();
},true);
