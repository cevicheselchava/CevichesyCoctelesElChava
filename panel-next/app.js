...
const blank = () => ({version:VERSION, inventory:[], recipes:[], orders:[], purchases:[], sales:[], expenses:[], prepared:[], updatedAt:0});
function normalize(data){
  ...
  return {...b,...data,version:VERSION,
    ...
    prepared:Array.isArray(data.prepared)?data.prepared:[],
...
function mapPublicOrder(snap){const d=snap.data()||{};return {
  ...
  items:Array.isArray(d.items)?d.items:[], publicRecipe:d.recipe&&typeof d.recipe==='object'?d.recipe:{}, payment:d.payment||'', notes:d.notes||'', inventoryApplied:!!d.inventoryApplied,
...
function preparedQty(recipeId){return Number(dbState.prepared.find(x=>x.recipeId===recipeId)?.qty||0);}
function changePrepared(recipeId,delta){if(!recipeId||!Number(delta))return;let row=dbState.prepared.find(x=>x.recipeId===recipeId);if(!row){row={recipeId,qty:0};dbState.prepared.push(row);}row.qty=Math.max(0,Number(row.qty||0)+Number(delta));dbState.prepared=dbState.prepared.filter(x=>Number(x.qty||0)>0.0001);}
function preparedOptions(){return sorted(dbState.recipes).filter(r=>preparedQty(r.id)>0).map(r=>[r.id,`${r.name} · ${qty(preparedQty(r.id))} lb listas`]);}
function qtyButtons(name,selected=1,max=Infinity){const opts=[0.5,1,1.5,2,2.5,3,4,5].filter(x=>x<=max+0.0001);return `<div class="choice-group" data-choice-group="${name}"><div class="choice-label">Cantidad</div><div class="choice-buttons">${opts.map(x=>`<button type="button" class="choice ${Number(selected)===x?'selected':''}" data-choice-name="${name}" data-choice-value="${x}">${qty(x)} lb</button>`).join('')}</div><input type="hidden" name="${name}" value="${esc(selected)}"></div>`;}
...
function renderHome(){
 ...
 $('#summary').innerHTML=`<div class="summary-title">Producción</div><div class="stats">
  <div><strong>${todayOrders}</strong><span>Pedidos hoy</span></div><div><strong>${tomorrowOrders}</strong><span>Pedidos mañana</span></div><div><strong>${active}</strong><span>Activos</span></div><div><strong>${low}</strong><span>Inventario bajo</span></div>
 </div><button type="button" class="secondary full" data-export>Respaldar datos</button>`;
}
...
function renderPrep(){const rows=allOrders().filter(o=>['pending','preparing'].includes(normalizeStatus(o.status))).sort((a,b)=>`${a.date||''} ${a.time||''}`.localeCompare(`${b.date||''} ${b.time||''}`));
 const grouped={};for(const o of rows){const k=o.date||'Sin fecha';(grouped[k]??=[]).push(o);} const readyStock=sorted(dbState.recipes).filter(r=>preparedQty(r.id)>0);
 const stockBlock=`<section class="prepared-box"><div class="section-head"><div><h3>Libras preparadas de más</h3><div class="muted">De aquí salen las ventas directas.</div></div><button type="button" class="secondary" data-extra-prep>＋ Preparar de más</button></div>${readyStock.length?`<div class="prepared-grid">${readyStock.map(r=>`<div class="prepared-chip"><strong>${esc(r.name)}</strong><span>${qty(preparedQty(r.id))} lb listas</span></div>`).join('')}</div>`:`<div class="muted">No hay libras extra preparadas.</div>`}</section>`;
 const ordersBlock=rows.length?Object.entries(grouped).map(([date,items])=>`<section class="day-section"><div class="day-title">${dayChip(date)} <strong>${qty(items.reduce((s,o)=>s+orderPounds(o),0))} lb</strong></div><div class="list">${items.map(o=>`<article class="row prep-row"><div class="row-head"><div><h3>${esc(orderProductText(o))}</h3><div class="muted">${esc(o.customer||'')} · ${esc(o.time||'')}</div></div><strong>${qty(orderPounds(o))} lb</strong></div><div class="actions">${normalizeStatus(o.status)==='pending'?`<button class="primary-inline" type="button" data-order-status="${o.id}" data-public="${o.public?'1':'0'}" data-status="preparing">Preparar</button>`:`<button class="primary-inline" type="button" data-order-status="${o.id}" data-public="${o.public?'1':'0'}" data-status="ready">Listo</button>`}</div></article>`).join('')}</div></section>`).join(''):empty('Nada por preparar.'); $('#content').innerHTML=stockBlock+ordersBlock;}
...
function needMapForDate(date){const needs=new Map();const orders=allOrders().filter(o=>o.date===date&&!['cancelled','delivered'].includes(normalizeStatus(o.status)));
 for(const o of orders){
   if(o.public && o.publicRecipe){for(const [key,val] of Object.entries(o.publicRecipe)){const inv=dbState.inventory.find(i=>i.appKey===key);if(!inv)continue;needs.set(inv.id,(needs.get(inv.id)||0)+Number(val||0));}continue;}
   const r=byId(dbState.recipes,o.recipeId); if(!r)continue; const mult=Number(orderPounds(o)||0)/Number(r.yieldLb||1); for(const line of r.ingredients||[]) needs.set(line.inventoryId,(needs.get(line.inventoryId)||0)+Number(line.qty||0)*mult);
 }
 return {needs,orders};
}
...
function renderSales(){const rows=[...dbState.sales].sort((a,b)=>`${b.date||''} ${b.time||''}`.localeCompare(`${a.date||''} ${a.time||''}`));const stock=sorted(dbState.recipes).filter(r=>preparedQty(r.id)>0);$('#content').innerHTML=`<div class="planner-note">Venta directa descuenta solamente de las libras ya preparadas de más. No vuelve a descontar ingredientes.</div>${stock.length?`<div class="prepared-grid sales-stock">${stock.map(r=>`<div class="prepared-chip"><strong>${esc(r.name)}</strong><span>${qty(preparedQty(r.id))} lb disponibles</span></div>`).join('')}</div>`:`<div class="warning">No hay libras preparadas de más disponibles para venta directa.</div>`}${rows.length?`<div class="section-head"><h3>Ventas directas</h3></div><div class="list">${rows.map(s=>`<article class="row"><div class="row-head"><h3>${esc(recipeName(s.recipeId)||s.product||'Venta')}</h3><strong>${money(s.total)}</strong></div><div class="muted">${esc(s.date||'')} ${esc(s.time||'')} · ${qty(s.qty||1)} lb</div></article>`).join('')}</div>`:''}`;}
...
function openExtraPrepForm(){if(!dbState.recipes.length){toast('Primero crea la receta del producto',2200);return;}showModal('Preparar de más',`${select('recipeId','Producto',sorted(dbState.recipes).map(r=>[r.id,r.name]))}${qtyButtons('qty',1,5)}`,fd=>{const recipeId=fd.get('recipeId'),q=Number(fd.get('qty')||0),r=byId(dbState.recipes,recipeId);if(!r||q<=0)return;applyRecipeInventory(r,q/Number(r.yieldLb||1));changePrepared(recipeId,q);saveLocal();closeModal();toast(`${qty(q)} lb agregadas a preparadas`);});}
function openSaleForm(){const opts=preparedOptions();if(!opts.length){toast('No hay libras preparadas de más para vender',2400);return;}const firstId=opts[0][0],max=preparedQty(firstId);showModal('Nueva venta',`${select('recipeId','Producto preparado',opts,firstId)}<div id="saleQtyChoices">${qtyButtons('qty',Math.min(1,max),max)}</div>${input('total','Total $','','number','min="0" step="0.01" required')}${select('date','Fecha',[[today(),'Hoy'],[tomorrow(),'Mañana']],today())}${input('time','Hora',nowTime(),'time','required')}`,fd=>{const recipeId=fd.get('recipeId'),q=Number(fd.get('qty')||0),available=preparedQty(recipeId);if(q<=0||q>available+0.0001){toast(`Solo hay ${qty(available)} lb preparadas`,2500);return;}changePrepared(recipeId,-q);dbState.sales.push({id:id(),recipeId,qty:q,total:Number(fd.get('total')||0),date:fd.get('date'),time:fd.get('time')});saveLocal();closeModal();toast('Venta registrada');});}
...
async function changeOrderStatus(orderId,status,isPublic){if(isPublic){const o=publicOrders.find(x=>x.id===orderId);if(!o)return;const firestoreStatus={pending:'nuevo',preparing:'preparando',ready:'listo',delivery:'en_entrega',delivered:'entregado',cancelled:'cancelado'}[status]||status;try{const updates={status:firestoreStatus};if(status==='ready'&&!o.inventoryApplied){applyPublicInventory(o);saveLocal();o.inventoryApplied=true;updates.inventoryApplied=true;}await updateDoc(doc(firestore,'pedidos',orderId),updates);toast('Pedido actualizado');}catch(e){console.error(e);toast('No se pudo actualizar',2500);}return;}
 const o=byId(dbState.orders,orderId);if(!o)return;if(status==='ready'&&!o.inventoryApplied){const r=byId(dbState.recipes,o.recipeId);if(r)applyRecipeInventory(r,Number(orderPounds(o)||0)/Number(r.yieldLb||1));o.inventoryApplied=true;}o.status=status;saveLocal();toast('Pedido actualizado');}
